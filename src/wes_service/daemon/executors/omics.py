...
class OmicsExecutor(WorkflowExecutor):
    ...
    async def _update_task_log_urls(self, db: AsyncSession, run_id: str, task_logs: Dict[str, str]) -> None:
        """
        ...
        try:
            ...
            if 'logLocation' in response:
                ...
                if 'runLogStream' in response['logLocation']:
                    ...
                    logger.warning(f"No runLogStream found in logLocation: {response['logLocation']}")
            else:
