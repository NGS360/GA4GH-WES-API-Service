...
class Settings(BaseSettings):
    ...
    #
    # Lets a trusted sibling service (NGS360 APIServer, which fronts this one
    # for the browser) authenticate with a shared key instead of a user token.
    # Without it the call chain is browser -> APIServer -> WES -> APIServer,
    # because validating a user token means asking APIServer /auth/me. That
    # re-entrant hop can exhaust APIServer's request threadpool under load, so
    # the facade path deliberately avoids it.
    #
    # Deliberately a SEPARATE secret from INTERNAL_CALLBACK_API_KEY: the
    # callback key is held by an AWS Lambda, this one by APIServer, and they
    # should be rotatable independently.
    enable_service_auth: bool = Field(
        default=True,
        description="Allow trusted sibling services to authenticate with a shared key",
    )

    @computed_field
    @property
    def INTERNAL_SERVICE_API_KEY(self) -> str:
        """Get the internal service-to-service API key from env or secrets"""
        return self._get_config_value(
